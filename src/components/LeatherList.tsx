...
import { Search, SlidersHorizontal, Filter, MoreVertical, Globe, Plus } from "lucide-react";
import { LeatherResponse, AvailabilityStatus } from "../types";
import { cn } from "../lib/utils";

interface LeatherListProps {
  leathers: LeatherResponse[];
  onEdit: (leather: LeatherResponse) => void;
  onDelete: (id: number) => void;
  onView: (leather: LeatherResponse) => void;
  onStatusChange: (id: number, status: AvailabilityStatus) => void;
  onCreate: () => void;
}

export default function LeatherList({
  leathers,
  onEdit,
  onDelete,
  onView,
  ...
  const filteredLeathers = (leathers || []).filter((l) => {
    const matchesSearch = l.leatherName.toLowerCase().includes(searchTerm.toLowerCase()) || 
                         l.origin.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === "ALL" || l.availabilityStatus === statusFilter;
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-8 bg-[#f0f3ff] rounded-2xl p-2 flex items-center gap-4 focus-within:bg-white transition-all shadow-sm">
          ...
          <input
            ...
            placeholder="Search premium hides, tanneries, or texture IDs..."
            type="text"
            value={searchTerm}
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="font-['Inter'] text-xs uppercase tracking-[0.15em] text-[#777587] mt-1">
            {filteredLeathers.length} Distinct Materials Identified
          </p>
        </div>
      </div>
                ...
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <select
                      ...
                      value={leather.availabilityStatus}
                      onChange={(e) => {
                        e.stopPropagation();
                        onStatusChange(leather.id, e.target.value as AvailabilityStatus);
                      }}
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span
                        className="w-3 h-3 rounded-full border border-[#777587]/20"
                ...
                <div className="mt-4 pt-4 border-t border-[#c7c4d8]/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={leather.isActive}
                        readOnly
                        className="sr-only peer"
                      />
                      <div className="w-9 h-5 bg-[#c7c4d8]/30 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#3525cd]"></div>
                      <span className="ml-3 text-xs font-bold uppercase tracking-widest text-[#111c2d]">
                        {leather.isActive ? "Active" : "Inactive"}
                      </span>
                    </div>
                  </div>
                  <button
